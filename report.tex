...
\item Mô hình một
\item Số tầng ẩn: 2
\item Tâng ẩn 1: 2048 neural
\item Tầng ẩn 2: 1024 neural
\item Hàm active: sigmod
\item Hàm score: sigmod
\item Hàm loss: $\frac{1}{2}\sum_{i=1}^n(y_i^2-p_i^2)<\infty$
\item Giải thuật học : lan truyền ngược:
\begin{itemize}
\item Công thức cập nhật tham số cho w tại tầng output: $\Delta w^{t}=\eta $
\item Công thức cập nhật tham số cho tầng 
\end{itemize }
\item Mô hình hai
\end{itemize}
\subsection{Kết quả}
